 ...
 * index structures that provide efficient access for both point queries and
 * range queries. All keys will be at the leaf level with leaf nodes linked by
 * references. Internal nodes will contain divider keys such that each divider
 * key corresponds to the smallest key in its right sub-tree (SMALLEST RIGHT).
 * Keys in left sub-tree are "<",
...
//-----------------------------------------------------------------------------------
     ...
     * Node key: [ . k0 . k1 . k2 . k3 . ] < <= <= <=   <=         note: number of keys is one less than number of refs
     *      ref:  r0   r1   r2   r3   r4
     * Leaf:      r0 -> next leaf node; r1 -> tuple (k0, ...); r2 -> tuple (k1, ...);
     * etc. Internal: r0 -> subtree with keys < k0; r1 -> sub-tree with keys in
     * [k0, k1); etc. Split: extra room in nodes allows the overflow key to be
    ...
    private class Node {
        ...
        Node(Node left, K dkey, Node right) {
            ...
            ref[0] = left;
            ref[1] = right;                            // left and right references
        } // constructor
        ...
        /**
         * **************************************************************************
         * Add the new key k and value v into this node at insertion position
         * (ip).
         *
         * @param k the new key
         ...
         * half the keys and references to that new node, leaving halfp. Return
         * the right sibling node, where the divider key is key[0].
         */
        Node split() {
            ...
            return rt;                                                  // (divider key (smallest right) in right sibling                                       // (divider key (smallest right) in right sibling
        } // split

        /**
         * **************************************************************************
         * Split this INTERNAL node by creating a right sibling rt and moving
         * half the keys and references to that new node, leaving halfp - 1.
...
//-----------------------------------------------------------------------------------
     ...
     * the key type to implement Comparable. public Comparator <? super K>
     * comparator () { return null; } // comparator
     */
    /**
     * ******************************************************************************
...
//-----------------------------------------------------------------------------------
    ...
    public Set<Map.Entry<K, V>> entrySet() {
        ...
        for (var n = firstLeaf; n != null; n = (Node) n.ref[0]) {
            ...
            for (var i = 0; i < n.keys; i++) {
                // Use ref[i + 1] to get the correct value
                enSet.add(new AbstractMap.SimpleEntry<K, V>(n.key[i], (V) n.ref[i + 1]));
            }
        }
        return enSet;
    }
    ...
    record NodePos(Object node, int pos) {

    }                          // as records are implicitly static, can't use 'Node node'
    ...
    /**
     * ******************************************************************************
     * Recursive helper method for finding the position of the given key in this
     * B+tree.
     *
     * @param key the key to find
...
//-----------------------------------------------------------------------------------
    ...
    /**
     * ******************************************************************************
     * Author: Thomas Recursive helper function for inserting a key into a
     * B+tree. Add key-ref pair into node n and when it is full will split node
     * n by allocating a right sibling node rt and placing the lesser half of
     * n's key in rt. A split also will require a key-ref pair to be inserted at
     * the next level up.
     *
     * @param key the key to insert
     ...
     * in which case the new right sibling node (containing the divider key) is
     * returned.
     *
     * @param n the current node
     * @param k the new key
    ...
    private Node add(Node n, K k, V v) {
        ...
        if (n.overflow()) {
            rt = n.split();                           // full => split into n and rt, divider key is rt.key[0]
     ...
     * Author: Thomas
     *
     * Add new key k and value v into INTERNAL node n. Upon overflow, split node
     * n, in which case the new right sibling node (containing the divider key)
     * is returned.
     *
     * @param n the current node
     * @param k the new key
...
//-----------------------------------------------------------------------------------
    ...
    private void printT(Node n, int level) {
        if (n != null) {
            ...
            if (!n.isLeaf) {
                for (var j = 0; j <= n.keys; j++) {
                    printT((Node) n.ref[j], level + 1);
                }
            } // if
        } // if
    } // printT
...
//-----------------------------------------------------------------------------------
    /**
     * ******************************************************************************
     * The main method used for testing. Also test for more keys and with
     * RANDOMLY true.
     *
     * @param the command-line arguments (args[0] gives number of keys to
     * insert)
     */
    public static void main(String[] args) {
        ...
        if (RANDOMLY) {
            ...
            for (var i = 1; i <= totalKeys; i += 2) {
                bpTree.put(rng.nextInt(2 * totalKeys), i * i);
            }
        } else {
            for (var i = 1; i <= totalKeys; i += 2) {
                bpTree.put(i, i * i);
